...
marketer_skills.each do|skill|
  ...
  new_skillset = Skillset.create!(skill: new_skill, active: true, skillable: marketer)
end

data_analyst_skills = SkillsSearchService.new.run(data_analyst.job_code)
data_analyst_skills.each do |skill|
  new_skill = Skill.where(name: skill)
  if new_skill == []
    new_skill = Skill.create!(name: skill)
  end
  new_skillset = Skillset.create!(skill: new_skill, active: true, skillable: data_analyst)
end
...
support_manager_skills.each do |skill|
  ...
  new_skillset = Skillset.create!(skill: new_skill, active: true, skillable: support_manager)
end

support_1_skills = SkillsSearchService.new.run(marketer.job_code)
support_1_skills.each do |skill|
  new_skill = Skill.where(name: skill)
  if new_skill == []
    new_skill = Skill.create!(name: skill)
  end
  new_skillset = Skillset.create!(skill: new_skill, active: true, skillable: support_1)
end

support_2_skills = SkillsSearchService.new.run(marketer.job_code)
support_2_skills.each do |skill|
  new_skill = Skill.where(name: skill)
  if new_skill == []
    new_skill = Skill.create!(name: skill)
  end
  new_skillset = Skillset.create!(skill: new_skill, active: true, skillable: support_2)
end
...
laura_data_walk = Walk.create!(walk: data_walk, user: laura, start_date: "01/01/2020")
john_data_walk = Walk.create!(walk: data_walk, user: john, start_date: "15/01/2020")
laetitia_support_walk = Walk.create!(walk: support_walk, user: laetita, start_date: "01/02/2020")
